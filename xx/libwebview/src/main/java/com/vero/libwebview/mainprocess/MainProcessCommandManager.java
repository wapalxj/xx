...
public class MainProcessCommandManager extends IWebviewProcessToMainProcessAidlInterface.Stub {
    ...
    @Override
    public void handleWebCommand(String commandName, String params) throws RemoteException {
        if (TextUtils.isEmpty(commandName)) {
            return;
        }
        getInstance().executeCommand(commandName, new Gson().fromJson(params, Map.class));
